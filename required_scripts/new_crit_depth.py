...
def Critical_Depth_ol(
    ...
    """
    Find depth of formation given olivine cooling rate.
    ...
    for ti in range(5, temperatures.shape[1]):
        ...
        index_where_1100K_ish = np.argmin(
            np.absolute(temperatures[:, ti] - 1273)
        )
    ...
    crossing_index2 = np.argmax(
        crosses
    )  # finds the first 'maximum' which is the first TRUE, or the first cross
    Critical_Radius = radii[
        dt_val[crossing_index2]
    ]  # radius where this first crossing occurs
    ...
    for ti in range(5, temperatures.shape[1]):
        # Find the index where temperatures are 593K by finding the minimum of
        # (a given temperature-593)
        index_where_600K_ish = np.argmin(
            np.absolute(temperatures[:, ti] - 600)
        )
        ...
        d_val.append(
            (Critical_Radius) / 1000 - ((r_planet / 1000) * core_size_factor)
        )  # computes the depth

    crossing = [
        np.array(t_val2) - d_val < 0.00001
    ]  # indicies where computed depth crosses temperature contour
    crossing_index = np.argmax(
        crossing
    )  # finds the first 'maximum' which is the first TRUE, or the first cross
    Time_of_Crossing = crossing_index * (10 ** 11)  # converts to seconds
    ...
    if time_core_frozen == 0:
        x = "Core Freezes after Max Time"
        depth = ((r_planet) - radii[int(((d_val)[crossing is True]))]) / 1000
        return (depth, x, time_core_frozen, Time_of_Crossing)
    else:
        if radii_index > len(radii):
            x = "Core has finished solidifying"
            ...
            depth = (
                (r_planet) - radii[int(((d_val)[crossing is True]))]
            ) / 1000
            if Time_of_Crossing == 0:
