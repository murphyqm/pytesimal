# modular function for timestepping to allow a variety of temperature dependent  functions OR constants
# Including the simple variable case
...
#    """
#    Function for temperature dependent density, defined above and using beta from Su et al., 2018
#    
#    Defaults for reference rho and T chosen from Bryson et al., 2015 with T0 set to room temperature
#    """
...
#def heat_cap(T):
#    cp =  (995.1 + (1343*((T)**(-0.5))) - (2.887*(10**7)*((T)**(-2))) - (6.166*(10**(-2))* (T)**(-3)))
#    return cp
...
def discretisation(latent,temp_init, temp_core_melting, temp_surface,
                 cmb_conductivity, p, c, temperatures, dr, coretemp, timestep,
                 core_density, core_cp, r_core, core_latent_heat, radii,
                 times, kappas, where_regolith,kappa_reg,
                 cond_constant="y",density_constant="y",heat_cap_constant = "y",non_lin_term="y"):
    """
    ...
    if cond_constant=="y":
        ...
        def conductivity(x):
            y = 80.4205952575632*(1.3193574749943*x**(-0.5) + 0.977581998039333 - 28361.7649315602/x**2.0 - 6.05745211527538e-5/x**3.0)*(1.0/x)**0.5
            return y
        def conductivity_prime(x):
            y_prime = 80.4205952575632*(-0.659678737497148*x**(-1.5) + 56723.5298631204/x**3.0 + 0.000181723563458261/x**4.0)*(1.0/x)**0.5 - 40.2102976287816*(1.3193574749943*x**(-0.5) + 0.977581998039333 - 28361.7649315602/x**2.0 - 6.05745211527538e-5/x**3.0)*(1.0/x)**0.5/x
            return y_prime
    ...
    if heat_cap_constant== "y":
        ...
        def heat_cap(T):
            cp =  (995.1 + (1343.0*((T)**(-0.5))) - (2.887*(10**7.0)*((T)**(-2.0))) - (6.166*(10.0**(-2.0))* (T)**(-3.0)))
            return cp
    ...
    for i in range(1, len(times[1:])+1):
        ...
        for j in range(1, len(radii[1:-1])+1):
            ...
            if where_regolith[j] == 1:
                ...
                if non_lin_term=="y":
                    A_1 = (timestep*(1.0/(rho(temperatures[j, i-1])* heat_cap(temperatures[j, i-1]))))*(conductivity_prime(temperatures[j, i-1]) * ((temperatures[j+1, i-1] - temperatures[j-1, i-1])**2)/(4.0 * dr**2.0))
                else:
                    A_1 = 0

                B_1 = (timestep*(1.0/(rho(temperatures[j, i-1])*heat_cap(temperatures[j, i-1]))))*((conductivity(temperatures[j,i-1])/(radii[j]*dr)) * (temperatures[j+1, i-1] - temperatures[j-1, i-1]))

                C_1 = (timestep*(1.0/(rho(temperatures[j, i-1])*heat_cap(temperatures[j, i-1]))))*((conductivity(temperatures[j,i-1])/dr**2.0) * (temperatures[j+1, i-1] -2*temperatures[j,i-1] + temperatures[j-1, i-1]))
...
### 20200526: chanign the nature of the simple variable function: will leave original just comented out
### see supporting work in python jupyter notebook
    
def simple_discretisation(latent,temp_init, temp_core_melting, temp_surface,
                 cmb_conductivity, p, c, B, temperatures, dr, coretemp, timestep,
                 core_density, core_cp, r_core, core_latent_heat, radii,
                 times, kappas, where_regolith,kappa_reg,
                 cond_constant="n",density_constant="y",heat_cap_constant = "y",non_lin_term="y"):
    """
    Finite difference solver with variable k from an experimentally derived function, separating the different terms
...
def comp_to_analytical_sep_terms(latent,temp_init, temp_core_melting, temp_surface,
    ...
    for i in range(1, len(times[1:])+1):
        ...
        for j in range(1, len(radii[1:-1])+1):
            ...
            A_1 = 0#(timestep*(1.0/(p*c)))*(dKdT(k_0[j],B) * ((temperatures[j+1, i-1] - temperatures[j-1, i-1])**2)/(4.0 * dr**2.0))
