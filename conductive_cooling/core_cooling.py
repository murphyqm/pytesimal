...
def core_freezing(coretemp, max_time, times, latent,temp_core_melting,timestep=1E11):
    ...
    core_frozen = [coretemp<=temp_core_melting] #creates boolean array for temp<=1200
    ...
    if time_core_frozen>=max_time or len(times_frozen) == 0.0:
        ...
        time_core_frozen = 0.0 #np.NaN #0
        fully_frozen = 0.0 #np.NaN #0
    else:
        time_core_frozen = times_frozen[2] #first time the temperature is less than 1200K # changed to one instead of zero.... might change back
    ...
    print("core_frozen: "+str(core_frozen)+ "; times_frozen: "+ str(times_frozen) + "; time_core_frozen: "+str(time_core_frozen)+ "; fully_frozen: "+ str(fully_frozen))
