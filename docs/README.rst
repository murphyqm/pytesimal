...
`Pytesimal` can also be installed and used in Google's hosted Jupyter notebook service, `Colaboratory <https://research.google.com/colaboratory/>`__. In a new notebook, install the package:

::

    !pip install pytesimal

You can also upload one of our `examples <https://pytesimal.readthedocs.io/en/latest/examples/>`__ and run it, after adding a cell with the installation command above to the start of the notebook.
